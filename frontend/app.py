...
# frontend/app.py - FIXED VERSION (CSS issue fixed)
import os
import streamlit as st
import requests
import json
...
# Modern AI-themed CSS with fixed styling
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Fira+Code:wght@400;500;600&family=Inter:wght@300;400;500;600;700&display=swap');
...
print(f"Average: {avg}")"""
        ...
        with col2:
            ...
            if st.session_state.analysis_result:
                ...
                # Display metrics - Using columns instead of raw HTML
                st.markdown("#### 📈 CODE METRICS")
                ...
                # Display roast - Using custom container
                st.markdown("#### 🔥 AI ROAST")
                roast = result.get('roast', {})
                roast_text = roast.get('text', 'No roast generated')
                intensity = roast.get('intensity', 'medium')
                
                # Create the roast box with proper HTML
                roast_html = f"""
                <div class="roast-box">
                    <div class="roast-content">
                        <p><strong>Intensity:</strong> <span class="intensity-badge intensity-{intensity}">{intensity.upper()}</span></p>
                        <p><strong>{roast_text}</strong></p>
                    </div>
                </div>
                """
                st.markdown(roast_html, unsafe_allow_html=True)
                
                # Audio buttons using Streamlit (not HTML)
                ...
                # Display issues - Using custom container
                issues = result.get('analysis', {}).get('issues', [])
                if issues:
                    st.markdown(f"#### ⚠️ ISSUES FOUND ({len(issues)})")
                    
                    # Create issues box
                    issues_html = '<div class="issues-box">'
                    for i, issue in enumerate(issues, 1):
                        issues_html += f'<div class="issue-item"><strong>{i}.</strong> {issue}</div>'
                    issues_html += '</div>'
                ...
                # Display suggestions - Using custom container
                suggestions = result.get('analysis', {}).get('suggestions', [])
                if suggestions:
                    st.markdown("#### 💡 SUGGESTIONS FOR IMPROVEMENT")
                ...
                st.info("👈 Paste your code and click 'ANALYZE & ROAST' to see results!")
                st.markdown("""
                <div style='text-align: center; padding: 2rem; background: rgba(30, 41, 59, 0.5); border-radius: 20px; border: 2px dashed var(--dark-border);'>
                    <div style='font-size: 4rem; margin-bottom: 1rem;'>🔥</div>
                    <h3>Ready to roast some code?</h3>
